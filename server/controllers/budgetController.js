...
const createBudget = async (req, res) => {
  try {
    ...
    if (!totalBudget) {
      return res
        .status(400)
        .json({ message: "Total budget amount is required" });
    }
...
const getBudget = async (req, res) => {
  try {
    ...
    if (!budget) {
      return res
        .status(404)
        .json({ message: "No budget set for this trip yet" });
    }

    const expenses = await Expense.find({ budgetId: budget._id }).sort({
      date: -1,
    });
...
const addExpense = async (req, res) => {
  try {
    ...
    if (!budget) {
      return res
        .status(404)
        .json({ message: "Set a budget for this trip before adding expenses" });
    }
